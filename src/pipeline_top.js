(params) => {
  ...
  const file_records = api.run("this.load_file", {
    file_url: params.file_url
  });
  const file_ids = file_records.map(r => {
    return r[order_id_index];
  });
  ...
  const existing_record_ids = existing_records.map(r => {
    return r[order_id_index];
  });

  const ids_to_be_processed = file_ids.filter(i => {
    return !existing_record_ids.includes(i);
  });
  ...
  const records_with_inventory = api.run("this.add_inventory", {
    data: records_with_sales_rep
  });
  // console.log(records_with_inventory);

  // any records with small inventory amounts, mark as such
  const records_with_inventory_warning = api.run("this.add_inventory_warning", {
    data: records_with_inventory
  });

  // push total profits to google sheets for exec dashboards
  const totals = api.run("this.generate_region_totals", {
    data: records_with_inventory_warning
  });
  // console.log(totals);

  let res = api.run("this.ship_totals_to_google_sheets", {
    totals: totals
  })[0];
  if (!res.success) {
